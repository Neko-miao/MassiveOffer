...
## 引用
### 左值引用和右值引用
- 左值引用指向左值，右值引用指向右值，但是const左值引用可以指向右值（常量）
- 右值没有地址，所以一般匿名变量认为是右值，将亡变量也是右值
- 右值引用可以通过std::move()强行指向左值，需要注意的是单纯std::move()只是实现一个类型转换，没有性能提升；
- 右值引用指向右值，本质也是把右值提升为左值（即可以修改）；
- 右值引用可以是左值也可以是右值，取决于右值引用有没有名称，或者说函数返回的&&是右值，直接声明的&&是左值；
- 右值引用和左值引用的联系：
    - 都是引用，在传参时可以避免深拷贝；
    - 都有办法既指向左值又指向右值；
    - 右值作为函数参数时，接受的右值是可以修改的，而const左值引用接受的参数是不能修改的；
- 右值引用的应用场景：
    - std::move()支持移动语义，避免深拷贝，从而提升性能；

### 完美转发  
- std::forwad()，可以转左值也可以转右值，其原型是std::forward<T>(u)，当T为左值引用类型时，u转为左值，当T为右值引用类型时，u转换为右值
...
- 联系：都是用来动态申请/释放内存空间的
- 区别：
    - malloc/free是C的标准库函数，而new/delete是C++的运算符；
    - malloc只负责分配内存，并返回一个void*指针，new不仅会分配内存，还会调用对象的构造函数；free是释放指针指向的空间，而delete会调用对象的析构函数，并释放内存，多个对象需要使用delete[]；
    - new会建立对象，可以通过对象访问的方式访问对象成员，而malloc分配的是一片内存区域，可以直接用指针访问，而且可以在里面移动指针；即new建立对象，而malloc分配内存；
    - new构建对象时是带类型信息的，可以自动计算所需内存大小，而malloc需要手动计算；
    - free释放空间时直接穿指针参数即可，如果P是NULL则多次释放不会有问题，但是如果P不是NULL，那么释放超过一次就会出问题，这说明，被free后的P并不是NULL，而称为了野指针；
    - new内置了sizeof、类型转换和类型安全检查功能，new一个对象数组时，只能用无参构造函数；
    - new是操作符，可以重载，可以自定义内存分配策略；
